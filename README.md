...
BimaGrid is a standalone, decentralized, multi-peril parametric insurance protocol engineered specifically for smallholder and subsistence farmers in East Africa. By combining flexible high-resolution spatial indexing (Uber H3), cloud-native satellite computing (openEO), independent Rust Oracle Nodes, and trustless smart contract execution, BimaGrid completely removes human bias, manual claims adjustment, and multi-month delays from climate insurance.
...
Turn satellite data into instant liquidity for the farmer who needs it most without a single human in the loop.
...
- Localized disasters such as hail and flash floods go uncompensated
...
- Mitigation discounts such as drip irrigation are unverifiable at scale
- Centralized databases are vulnerable to manipulation
...
Technology: Django + PostGIS + openEO
...
Technology: Rust network with 3 independent nodes
...
Technology: Smart contract + escrow
...
Configuration example (`oracle-config-1.toml`):
...
- KilimaShieldOracle.sol - Oracle data receiver and consensus
- PolicyRegistry.sol - Policy minting and management
- EscrowVault.sol - Premium pooling and payout execution
- MitigationVerifier.sol - Discount validation

Deployment addresses on localhost:
...
```text
CON Welcome to BimaGrid. 1. Register Farm 2. Check Policy Status 3. File Claim
```
...
Internal API: `POST /oracles/submit-data/`

Headers:

- X-Oracle-Signature: {ecdsa_sig}
...
Webhook from smart contract: `POST /payments/payout-trigger/`
...
- When 3 submissions are received, calculates median
...
- Median calculation uses sorting network with no loops
...
1. Scheduler wakeup at 23:00 EAT daily
   - Tokio interval timer triggers evaluation cycle

2. Data ingestion through parallel async tasks
...
farm_mean_ndwi = ndwi.aggregate_spatial(geometries=farm_polygon_geojson, reducer="mean")
...
- Receive M-Pesa SMS:
  ...
  "BimaGrid Alert: Drought detected in your area. KES 5,000 sent to your M-Pesa account. Tx: 0xabc123... Hash: 0xdef456..."
